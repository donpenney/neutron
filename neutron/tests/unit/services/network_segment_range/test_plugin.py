...
class TestNetworkSegmentRange(testlib_api.SqlTestCase):
    ...
    def test__is_network_segment_range_referenced(self):
        with mock.patch.object(self.plugin.type_manager,
                               'network_segments_exist',
                               return_value=True):
    ...
    def test__is_existing_network_segment_range_impacted(self):
        ...
        for ret in impacted_existing_ranges:
            with mock.patch.object(self.plugin.type_manager,
                                   'network_segment_existing_range',
                                   return_value=ret):
                self.assertTrue(
                    self.plugin._is_existing_network_segment_range_impacted(
                        self.context, existing_range, updated_range))

    def test__is_existing_network_segment_range_unimpacted(self):
        existing_range = self._foo_range
        updated_range = self._vlan_range
        with mock.patch.object(self.plugin.type_manager,
                               'network_segment_existing_range',
    ...
    def test_update_network_segment_range(self):
        ...
        with mock.patch.object(self.plugin,
                               '_is_existing_network_segment_range_impacted',
                               return_value=False):
