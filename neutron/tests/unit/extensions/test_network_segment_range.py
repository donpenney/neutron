...
from neutron.services.network_segment_range import db
...
class NetworkSegmentRangeTestCase(test_db_base_plugin_v2.
    ...
    def _make_network_segment_range(self, fmt, **kwargs):
        res = self._create_network_segment_range(fmt, **kwargs)
        if res.status_int >= webob.exc.HTTPClientError.code:
            raise webob.exc.HTTPClientError(code=res.status_int)
        return self.deserialize(fmt, res)

    def network_segment_range(self, **kwargs):
        network_segment_range = self._make_network_segment_range(self.fmt,
                                                                 **kwargs)
        return network_segment_range

    def _test_create_network_segment_range(self, expected=None, **kwargs):
        keys = kwargs.copy()
        network_segment_range = self.network_segment_range(**keys)
        self._validate_resource(network_segment_range, keys,
...
class TestNetworkSegmentRange(NetworkSegmentRangeTestCase):
    ...
    def test_update_network_segment_range_set_name(self):
        ...
        self.plugin.type_manager.network_segment_existing_range.return_value\
            = (None, None)
        result = self._update(
        ...
        self.assertEqual('foo-name', result['network_segment_range']['name'])
    ...
    def test_list_network_segment_ranges(self):
        ...
        self._test_create_network_segment_range(name='foo-range2')
    ...
    def test_delete_network_segment_range(self):
        network_segment_range = self._test_create_network_segment_range()
        self.plugin.type_manager.network_segments_exist.return_value = False
