...
class SegmentTypeDriver(BaseTypeDriver):
    ...
    def build_segment_query_with_network_segment_range(self, session,
                                                       **filters):
        ...
        if network_type == p_const.TYPE_VLAN:
            query = query.join(
                ...
                and_(range_model.NetworkSegmentRange.network_type ==
                     self.get_type(),
                     self.model.physical_network ==
                     range_model.NetworkSegmentRange.physical_network))
        else:
            query = query.join(
                range_model.NetworkSegmentRange,
                and_(range_model.NetworkSegmentRange.network_type ==
        ...
        query_project_id = query.filter(
            range_model.NetworkSegmentRange.project_id == project_id)
    ...
    def allocate_partially_specified_segment(self, context, **filters):
        ...
        session, ctx_manager = self._get_session(context)
        with ctx_manager:
            query = (self.build_segment_query_with_network_segment_range(
                          session, **filters)
                     if directory.get_plugin('network-segment-range') else
